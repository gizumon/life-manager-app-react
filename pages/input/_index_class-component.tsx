/**
 * React class component sample for study
 */
...
class InputComponent extends Component<Props, IState> {
  ...
  tabMap: ITabMap;
  tabIndex: ITabIndex | undefined;
  constructor(props: any) {
    super(props);
    this.useStyles = makeStyles((theme: Theme) => createStyles({
      root: {
        flexGrow: 1,
  ...
  initialize() {
    ...
    this.tabIndex = this.type === undefined ? 0 : this.tabMap.toIndex[this.type];
  }
  onTabChange(_: React.ChangeEvent<{}>, newValue: ITabIndex) {
    // setValue(newValue);
    this.setState({
      tabIndex: newValue,
      selectedConfig: configs.find((config) => config.type === this.tabMap.toType[newValue]) || configs[this.tabMap.toIndex['pay']],
    });
  }
  makeTabProps(index: number) {
    return {
        ...
        <Box display="flex" justifyContent="center">
          <Card className={this.classes?.card}>
            <Tabs
              ...
              aria-label="nav tabs"
            >
              <Tab label={<><PaymentIcon /> Pay</>} {...this.makeTabProps(this.tabMap.toIndex['pay'])} />
              <Tab label={<><AssignmentTurnedInIcon /> ToDo</>} {...this.makeTabProps(this.tabMap.toIndex['todo'])} />
              <Tab label={<><AddShoppingCartIcon /> ToBuy</>} {...this.makeTabProps(this.tabMap.toIndex['tobuy'])} />
            </Tabs>
            <CardContent>
                {
                  this.selectedConfig?.inputs.map((input) => {
